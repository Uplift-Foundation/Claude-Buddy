...
namespace ClaudeBuddy
{
    ...
    public static class TextToSpeech
    {
        ...
        // Preparing exists because the neural engine has a real wait in front of
        // its first sound — process start, a model load, then the first segment's
        // synthesis, measured at ~3.3s — and a stop button shown over silence
        // reads as a hang. The system voices go straight to Speaking; they start
        // in about half a second.
        public enum SpeakState { Idle, Preparing, Speaking }

        private static SpeakState _state = SpeakState.Idle;

        // Every state change, so the orb flyout's button can follow along.
        //
        // This replaces polling that never existed: IsSpeaking used to be derived
        // from "is the child process still alive", and because nothing asked it
        // again after a click, speech that finished on its own left the stop glyph
        // showing until the flyout was reopened. Reporting the change fixes that
        // on both platforms, and is what makes Preparing visible at all.
