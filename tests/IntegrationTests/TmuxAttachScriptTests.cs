...
public class TmuxAttachScriptTests
{
    ...
    public void ADirectoryFullOfShellMetacharactersParses()
    {
        AssertParses(TerminalScripts.TmuxAttachScript(
            "/usr/bin/tmux", "/tmp/tmux-501/default", "claude-swarm",
            "/Users/user/a \"quoted\" $HOME `whoami` dir"));
    }
    ...
    {
        AssertParses(TerminalScripts.TmuxAttachScript(
            "/usr/bin/tmux", "/tmp/user's sockets/claude-swarm-1", "claude-swarm", "/tmp"));
    }
    ...
    public void TheSplitCommandParses()
    {
        AssertCommandParses(TerminalScripts.TmuxSplitArgs(
            null, "user:3", "/Users/user/project",
            "'/Users/user/.local/bin/claude' attach '0e043819'"));
    }
    ...
    public void ASplitCommandFullOfShellMetacharactersParses()
    {
        AssertCommandParses(TerminalScripts.TmuxSplitArgs(
            null, "user:3", "/tmp",
            TerminalScripts.ShellQuote("/Users/user/a \"quoted\" $HOME `whoami`/claude")
    ...
    {
        AssertParses(TerminalScripts.TmuxAttachScript(
            "/usr/bin/tmux", "/tmp/tmux-501/default", "user's session", "/tmp"));
    }
    ...
    {
        AssertParses("#!/bin/sh\n" + TerminalScripts.TmuxAttachCommand(
            "/usr/bin/tmux", "/tmp/user's sockets/s", "user's session") + "\n");
    }
}
