...
public class OpenClawNameLookupTests
{
    ...
    // Keyed by the BARE agent id — "nova", not "openclaw:agent:nova:main".
    ...
    {
        Identities(("nova", "Nova"), ("kai", "Kai"));

        Assert.NotNull(OpenClawSessions.AvatarForAgentName("Nova"));
    }
    ...
    {
        Identities(("nova", "Nova"), ("nova2", "Nova"));

        Assert.Null(OpenClawSessions.AvatarForAgentName("Nova"));
    }
    ...
    // per agent — see the test below. Reusing "nova" here returned the face an
    ...
    {
        var session = new OpenClawChatSession("openclaw:agent:nova:main", "nova", "Nova");
