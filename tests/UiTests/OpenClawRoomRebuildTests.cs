...
public class OpenClawRoomRebuildTests
{
    ...
    public void AMemberSpeakingReachesTheRoomOnItsOwn()
    {
        ...
        Give(nova, (ChatRole.Assistant, "the build is green", 1));
    ...
    public void ManyChangesInOnePassCostOneRebuild()
    {
        ...
        for (var i = 0; i < 10; i++)
        {
            Give(nova, (ChatRole.Assistant, "still typing " + i, 1));
        }
    ...
    public void AChangeAfterTheQueueDrainsSchedulesAnotherRebuild()
    {
        ...
        Give(nova, (ChatRole.Assistant, "first", 1));
        Dispatcher.UIThread.RunJobs();

        Give(nova, (ChatRole.Assistant, "second", 2));
        Dispatcher.UIThread.RunJobs();
    ...
    public void OlderHistoryArrivingOnTheFrontTriggersARebuild()
    {
        var nova = Member("nova");
        nova.HasMore = false;
        Give(nova, (ChatRole.Assistant, "recent", 10));
        var room = Room((nova, "Nova", "#ff0000"));
