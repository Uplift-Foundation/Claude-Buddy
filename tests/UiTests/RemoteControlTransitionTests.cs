...
public class RemoteControlTransitionTests : IDisposable
{
    ...
    {
        var seen = Transitions(Remote("nova", "running"));

        Assert.Equal(new[] { "rc:work@example.com:nova=True" }, seen);
    }
    ...
    {
        Assert.Empty(Transitions(Remote("nova", "idle")));
    }
    ...
    {
        Transitions(Remote("nova", "running"));

        Assert.Empty(Transitions(Remote("nova", "running")));
    }
    ...
    public void ASessionThatStopsIsReportedOnce()
    {
        ...
        Assert.Equal(new[] { "rc:work@example.com:nova=False" },
            Transitions(Remote("nova", "idle")));
    ...
    public void TwoAccountsWithTheSameSessionNameAreTrackedSeparately()
    {
        var seen = Transitions(
            Remote("nova", "running", "work@example.com"),
            Remote("nova", "running", "home@example.com"));

        Assert.Equal(2, seen.Count);
        Assert.Contains("rc:work@example.com:nova=True", seen);
        Assert.Contains("rc:home@example.com:nova=True", seen);
    }

    [AvaloniaFact]
    public void SeveralSessionsChangingAtOnceAreAllReported()
    {
        Transitions(Remote("nova", "running"), Remote("kai", "running"));

        var seen = Transitions(Remote("nova", "idle"), Remote("kai", "idle"));
    ...
    public void RepublishFlattensEveryRelaysSessions()
    {
        RemoteControlSessions.SetRelayForTests("work@example.com", "1 session",
            sessions: new[] { Remote("nova", "idle", "work@example.com") });
    ...
    public void AColourLearnedLaterReachesAnAlreadyPublishedSession()
    {
        ...
        RemoteControlSessions.OnMessage("work@example.com",
            new BridgeProtocol.InboundMessage(
                FromName: "nova", From: "bridge:session_01", Mode: "prompting",
