...
public class SessionPresenceTests
{
    ...
    private static SessionPresence.ViewerPane Pane(
        string id, string window = "user:1", bool active = true, bool claimed = false) =>
    ...
    public void NoCandidatesIsNoViewer()
    {
        var (verdict, pane) = SessionPresence.ViewerAmong(
            Array.Empty<SessionPresence.ViewerPane>(), "user:1", anyClientAttached: true);
    ...
    public void TheActivePaneOfTheUsersOwnWindowMeansTheyAreLookingAtIt()
    {
        var (verdict, _) = SessionPresence.ViewerAmong(
            new[] { Pane("%6", "user:1", active: true) }, "user:1", anyClientAttached: true);
    ...
    {
        var (verdict, pane) = SessionPresence.ViewerAmong(
            new[] { Pane("%7", "user:1", active: false) }, "user:1", anyClientAttached: true);
    ...
    {
        var (verdict, pane) = SessionPresence.ViewerAmong(
            new[] { Pane("%6", "user:1", active: true) }, usersWindow, anyClientAttached: true);
    ...
    {
        var (verdict, pane) = SessionPresence.ViewerAmong(
            new[] { Pane("%53", "claude-swarm:1", active: true, claimed: true) }, "user:1",
    ...
    {
        var (verdict, _) = SessionPresence.ViewerAmong(
            new[] { Pane("%6", "user:1", active: true, claimed: true) }, "user:1",
            anyClientAttached: true);
