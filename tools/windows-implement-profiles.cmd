@echo off
rem Runs the Windows profile-switcher implementation (docs/windows-profile-switcher.md).
rem
...
setlocal
set LOG=C:\Users\warre\cbprofiles.log
set REPO=C:\cb
...
git rev-parse --verify windows-profile-switcher >nul 2>&1 || git branch windows-profile-switcher %BASE% >> %LOG% 2>&1
git checkout windows-profile-switcher >> %LOG% 2>&1

"C:\Users\warre\.local\bin\claude.exe" -p "Read docs/windows-profile-switcher.md and implement it. You are running unattended: nobody will answer questions, so make judgement calls and record them rather than waiting for input." --allowedTools "Bash,Read,Write,Edit,Glob,Grep,TodoWrite" --output-format text >> %LOG% 2>&1
