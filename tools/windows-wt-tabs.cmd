@echo off
rem Runs the Windows Terminal tab-selection work (docs/windows-wt-tabs.md).
rem
...
setlocal
set LOG=C:\Users\warre\cbwttabs.log
set REPO=C:\cb
...
git rev-parse --verify windows-wt-tabs >nul 2>&1 || git branch windows-wt-tabs %BASE% >> %LOG% 2>&1
git checkout windows-wt-tabs >> %LOG% 2>&1

"C:\Users\warre\.local\bin\claude.exe" -p "Read docs/windows-wt-tabs.md and do it. You are running unattended: nobody will answer questions, so make judgement calls and record them rather than waiting for input." --allowedTools "Bash,Read,Write,Edit,Glob,Grep,TodoWrite" --output-format text >> %LOG% 2>&1
