@echo off
rem Runs the Windows quit-and-focus work (docs/windows-quit-and-focus.md).
rem
...
setlocal
set LOG=C:\Users\warre\cbquitfocus.log
set REPO=C:\cb
...
git rev-parse --verify windows-quit-focus >nul 2>&1 || git branch windows-quit-focus %BASE% >> %LOG% 2>&1
git checkout windows-quit-focus >> %LOG% 2>&1

"C:\Users\warre\.local\bin\claude.exe" -p "Read docs/windows-quit-and-focus.md and do it. You are running unattended: nobody will answer questions, so make judgement calls and record them rather than waiting for input." --allowedTools "Bash,Read,Write,Edit,Glob,Grep,TodoWrite" --output-format text >> %LOG% 2>&1
