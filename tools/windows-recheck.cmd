@echo off
rem Runs the Windows re-check of two blind fixes (docs/windows-recheck.md).
rem
...
setlocal
set LOG=C:\Users\warre\cbrecheck.log
set REPO=C:\cb
...
git rev-parse --verify windows-recheck >nul 2>&1 || git branch windows-recheck %BASE% >> %LOG% 2>&1
git checkout windows-recheck >> %LOG% 2>&1

"C:\Users\warre\.local\bin\claude.exe" -p "Read docs/windows-recheck.md and do it. You are running unattended: nobody will answer questions, so make judgement calls and record them rather than waiting for input." --allowedTools "Bash,Read,Write,Edit,Glob,Grep,TodoWrite" --output-format text >> %LOG% 2>&1
